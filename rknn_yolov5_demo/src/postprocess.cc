...
const int anchor0[6] = ANCHOR0;
const int anchor1[6] = ANCHOR1;
const int anchor2[6] = ANCHOR2;
...
static int process1(uint8_t *input, int *anchor, int grid_h, int grid_w, int height, int width, int stride,
...
int post_process(uint8_t *input0, uint8_t *input1, uint8_t *input2, int model_in_h, int model_in_w,
                 ...
                 std::vector<uint8_t> &qnt_zps, std::vector<float> &qnt_scales, detect_result_group_t *group) {
    static int init = -1;
    if (init == -1) {
        int ret = 0;
